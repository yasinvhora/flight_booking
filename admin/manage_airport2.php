...
if(isset($_GET['id']) && $_GET['id']!=''){
	...
	$result=$obj->getData('airport_list_2','*',$condition_arr,'','','','');
...
if(isset($_POST['submit'])){
	...
	$condition_arr=array('city_id'=>$city_id,'airport'=>$airport,'added_on'=>$added_on,'updeted_on'=>$updated_on);
		...
		if($id==''){
		$obj->insertData('airport_list_2',$condition_arr);
	}else{
		$obj->updateData('airport_list_2',$condition_arr,'id',$id);
	}
	...
	<script>
	alert("Success");
	window.location.href='airport2.php';
	</script>
	<?php
}
